...
class UserService:
    ...
    def update_user(
        ...
        try:
            ...
            if email:
                current_app.logger.info(
                    f"Пользователю {user.username} изменен email на '{email}'"
                )
    ...
    def toggle_admin_mode(user_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            db.session.commit()
            current_app.logger.info(
                f"Пользователь {user.username} переключен в режим {mode}"
            )
    ...
    def toggle_subscription(user_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            db.session.commit()
            current_app.logger.info(
                f"Подписка для пользователя {user.username} {status}"
            )
