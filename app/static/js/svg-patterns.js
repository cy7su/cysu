    ...
    generateWavesPattern() {
        ...
        for (let i = 0; i < numWaves; i++) {
            ...
            if (waveType === 'gradient') {
                ...
                waves.push(
                    `<path d="${path1}" fill="none" stroke="${color}" stroke-width="${strokeWidth}" opacity="${opacity}"/>`,
                )
                ...
                const path2 = this.generateWavePath(
                    size.width,
                    secondaryYOffset,
                    secondaryAmplitude,
                    secondaryFrequency,
                ...
                waves.push(
                    `<path d="${path2}" fill="none" stroke="${secondaryColor}" stroke-width="${strokeWidth * 0.7}" opacity="${secondaryOpacity}"/>`,
                )
                ...
                waves.push(
                    `<path d="${path}" fill="none" stroke="${color}" stroke-width="${strokeWidth}" opacity="${opacity}"/>`,
                )
            } else {
                // Простая волна
        ...
        for (let level = 0; level < numLevels; level++) {
            ...
            for (let branch = 0; branch < numBranches; branch++) {
                ...
                if (elementType === 0) {
                    ...
                    fractal.push(
                        `<path d="${spiralPoints}" stroke="${color}" stroke-width="${2 + level}" fill="none" opacity="${opacity}"/>`,
                    )
                } else if (elementType === 1) {
                    const circleRadius = branchLength * 0.3
                    fractal.push(
                        `<circle cx="${endX}" cy="${endY}" r="${circleRadius}" fill="${color}" opacity="${opacity}"/>`,
                    )
                    ...
                    for (let i = 0; i < 4; i++) {
                        ...
                        fractal.push(
                            `<circle cx="${innerX}" cy="${innerY}" r="${circleRadius * 0.3}" fill="${innerColor}" opacity="${opacity * 0.7}"/>`,
                        )
                    }
        ...
        for (let layer = 0; layer < numLayers; layer++) {
            ...
            if (effectType === 0) {
                ...
                for (let i = 0; i < numWaves; i++) {
                    ...
                    optical.push(
                        `<polyline points="${wavePoints.join(' ')}" stroke="${color}" stroke-width="${3 + layer}" fill="none" opacity="${opacity}"/>`,
                    )
                }
                ...
                for (let spiral = 0; spiral < numSpirals; spiral++) {
                    ...
                    optical.push(
                        `<path d="${spiralPoints}" stroke="${color}" stroke-width="${2 + layer}" fill="none" opacity="${opacity}"/>`,
                    )
                }
            } else {
                ...
                for (let i = 0; i < numShapes; i++) {
                    ...
                    if (shapeType === 0) {
                        ...
                        optical.push(
                            `<circle cx="${x + shapeSize / 2}" cy="${y + layerHeight / 2}" r="${circleRadius}" fill="${color}" opacity="${opacity}"/>`,
                        )
                        ...
                        for (let k = 0; k < 6; k++) {
                            ...
                            optical.push(
                                `<circle cx="${innerX}" cy="${innerY}" r="${circleRadius * 0.2}" fill="${innerColor}" opacity="${opacity * 0.8}"/>`,
                            )
                        }
                    }
                }
                ...
                for (let col = 0; col < cols; col++) {
                    ...
                    if (x < size.width && y < size.height) {
                        ...
                        if (elementType === 0) {
                            ...
                            mosaic.push(
                                `<polygon points="${points.join(' ')}" fill="${color}" opacity="${layerOpacity}"/>`,
                            )
                        } else if (elementType === 1) {
                            const numPoints = this.randomInt(5, 8)
                            ...
                            mosaic.push(
                                `<circle cx="${x + cellSize / 2}" cy="${y + cellSize / 2}" r="${circleRadius}" fill="${color}" opacity="${layerOpacity}"/>`,
                            )
                            ...
                            for (let k = 0; k < 4; k++) {
                                ...
                                mosaic.push(
                                    `<circle cx="${innerX}" cy="${innerY}" r="${circleRadius * 0.2}" fill="${innerColor}" opacity="${layerOpacity * 0.8}"/>`,
                                )
                            }
                        } else {
                            const centerX = x + cellSize / 2
                            ...
                            const innerColor = palette[Math.floor(Math.random() * palette.length)]
                            mosaic.push(
                                `<circle cx="${centerX}" cy="${centerY}" r="${diamondSize / 4}" fill="${innerColor}" opacity="${layerOpacity * 0.7}"/>`,
                            )
                        }
                    }
                }
        ...
        for (let knot = 0; knot < numKnots; knot++) {
            ...
            if (knotType === 0) {
                ...
                for (let arm = 0; arm < numArms; arm++) {
                    ...
                    celtic.push(
                        `<path d="${spiralPoints}" stroke="${color}" stroke-width="${3}" fill="none" opacity="${opacity}"/>`,
                    )
                }
            } else if (knotType === 1) {
                ...
                for (let ring = 0; ring < numRings; ring++) {
                    ...
                    for (let i = 0; i < numElements; i++) {
                        ...
                        if (elementType === 0) {
                            celtic.push(
                                `<circle cx="${x}" cy="${y}" r="${elementSize}" fill="${color}" opacity="${opacity}"/>`,
                            )
                            ...
                            for (let k = 0; k < 4; k++) {
                                ...
                                celtic.push(
                                    `<circle cx="${innerX}" cy="${innerY}" r="${elementSize * 0.3}" fill="${innerColor}" opacity="${opacity * 0.8}"/>`,
                                )
                            }
                        } else {
                            const numPoints = this.randomInt(5, 8)
                ...
                for (let row = 0; row < rows; row++) {
                    for (let col = 0; col < cols; col++) {
                        ...
                        if (
                            x >= centerX - knotSize / 2 &&
                            x <= centerX + knotSize / 2 &&
                            y >= centerY - knotSize / 2 &&
                            y <= centerY + knotSize / 2
                        ) {
                            const color = palette[Math.floor(Math.random() * palette.length)]
                            ...
                            if (shapeType === 0) {
                                ...
                                const circleRadius = cellSize * 0.4
                                celtic.push(
                                    `<circle cx="${x + cellSize / 2}" cy="${y + cellSize / 2}" r="${circleRadius}" fill="${color}" opacity="${opacity}"/>`,
                                )
