...
class UserService:
    ...
    def create_user(username: str, email: str, password: str,
                   ...
                   group_id: Optional[int] = None) -> Tuple[Optional[User], str]:
    ...
    def _delete_user_related_data(user_id: int, username: str) -> None:
        ...
        try:
            ...
            current_app.logger.info(f"Удалено уведомлений пользователя {username}: {notifications_count}")

            # Удаляем сообщения тикетов
            ticket_messages_count = TicketMessage.query.filter_by(user_id=user_id).delete()
            current_app.logger.info(f"Удалено сообщений тикетов пользователя {username}: {ticket_messages_count}")
            ...
            current_app.logger.info(f"Удалено кодов подтверждения email пользователя {username}: {email_verifications_count}")
            ...
            current_app.logger.info(f"Удалено сообщений чата пользователя {username}: {chat_messages_count}")

        except Exception as e:
            current_app.logger.error(f"Ошибка удаления связанных данных пользователя {username}: {e}")
    ...
    @staticmethod
    def change_user_group(user_id: int, group_id: Optional[int], new_group_id: Optional[int]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if new_group_id:
                ...
                db.session.commit()
                return True, f"Пользователь {user.username} перемещен в группу '{group.name}'"
            else:
                user.group_id = None
                db.session.commit()
    ...
    @staticmethod
    def change_user_status(user_id: int, current_user_id: int, new_role: str, admin_mode_enabled: bool = False) -> Tuple[bool, str]:
    ...
    @staticmethod
    def mass_change_group(user_ids: List[int], group_id: Optional[int], current_user_id: int) -> Tuple[int, str]:
    ...
    @staticmethod
    def mass_change_status(user_ids: List[int], status: str, current_user_id: int) -> Tuple[int, str]:
