...
class UserManagementService:
    ...
    def has_active_subscription(user: User) -> bool:
        ...
        # Проверяем ручную подписку
        if user.is_manual_subscription and user.subscription_expires and user.subscription_expires > datetime.utcnow():
            return True

        # Проверяем пробную подписку
        if user.is_trial_subscription and user.trial_subscription_expires and user.trial_subscription_expires > datetime.utcnow():
            return True

        # Проверяем обычную подписку
        return user.is_subscribed and user.subscription_expires and user.subscription_expires > datetime.utcnow()
    ...
    def create_group(name: str, description: str = None) -> Optional[Group]:
        ...
        try:
            if Group.query.filter_by(name=name).first():
                return None  # Группа с таким именем уже существует
