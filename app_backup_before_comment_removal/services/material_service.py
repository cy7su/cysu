...
class MaterialService:
    ...
    def get_subject_materials(subject_id: int) -> Tuple[List[Material], List[Material]]:
        ...
        assignments = Material.query.options(joinedload(Material.submissions)).filter_by(subject_id=subject_id, type="assignment").all()
        return lectures, assignments

    @staticmethod
    def create_material(subject_id: int, title: str, description: str, material_type: str,
                       file_data=None, solution_file_data=None, created_by: int = None) -> Material:
        filename = None
        solution_filename = None

        if file_data:
            full_path, relative_path = FileStorageManager.get_material_upload_path(subject_id, get_safe_filename(file_data.filename))
    ...
    def replace_material_file(material: Material, file_data) -> bool:
        ...
        if hasattr(file_data, 'content_length') and file_data.content_length:
            if file_data.content_length > current_app.config.get('MAX_CONTENT_LENGTH', 500 * 1024 * 1024):
                return False
        ...
        full_path, relative_path = FileStorageManager.get_material_upload_path(material.subject_id, filename)
    ...
    def submit_solution(material: Material, user_id: int, file_data) -> bool:
        ...
        full_path, relative_path = FileStorageManager.get_subject_upload_path(material.subject_id, user_id, filename)
