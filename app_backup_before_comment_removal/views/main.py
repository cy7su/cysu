...
from flask import Blueprint, Response, current_app, flash, redirect, render_template, request, url_for, send_file, session
from flask_login import current_user, login_required
from sqlalchemy.orm import joinedload

from ..forms import MaterialForm
from ..services import UserManagementService
from ..models import SiteSettings, SubjectGroup
...
from .context_processors import inject_admin_users, inject_json_parser, inject_moment, inject_subscription_status, inject_timestamp
...
def index() -> Union[str, Response]:
    ...
    if current_user.is_authenticated:
        ...
        if current_user.is_admin and request.method == "POST" and request.form.get("title"):
            try:
                ...
                if not pattern_svg:
                    pattern_svg = "default_dots"  # Упрощено
    ...
    try:
        if current_user.is_authenticated:
            subjects = UserManagementService.get_accessible_subjects(current_user)
...
def subject_detail(subject_id: int) -> Union[str, Response]:
    ...
    form = None
    if current_user.is_authenticated and UserManagementService.can_add_materials_to_subject(current_user, subject):
        form = MaterialForm()
        ...
        if form and form.validate_on_submit():
            material = MaterialService.create_material(
                ...
                solution_file_data=form.solution_file.data if form.type.data == "assignment" else None,
                created_by=current_user.id
            )
    ...
    if current_user.is_authenticated:
        can_add_materials = UserManagementService.can_add_materials_to_subject(current_user, subject)
        can_manage_materials = UserManagementService.can_manage_subject_materials(current_user, subject)
